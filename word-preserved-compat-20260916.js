...
(function(){
'use strict';
const industryIconRoot=new URL('img/',document.currentScript?.src||location.href).href;
...
function replaceFAQ(path){
 ...
 const heading=[...document.querySelectorAll('h2')].find(e=>/よくあるご質問/.test(e.textContent));if(!heading||heading.closest('[data-word-revision]'))return;
...
function init(){
 ...
 if(path==='/industry'){
  document.querySelector('.section_indlist')?.remove();
  ['manufacturing','saas','ec'].forEach(slug=>{
   const card=document.querySelector('.nove-industry-link[href="/industry/'+slug+'"]');
   if(!card||card.querySelector('.nove-industry-detail-icon'))return;
   const icon=el('img','nove-industry-detail-icon');
   icon.src=industryIconRoot+'industry-detail-'+slug+'.svg';icon.alt='';icon.width=160;icon.height=112;icon.loading='lazy';icon.decoding='async';
   card.prepend(icon);
  });
 }
 ...
 const absent=[]; // All 9 industry pages are approved for implementation; preserve their links.
 ...
 document.querySelectorAll('.nove-related-card').forEach(a=>{if(a.closest('[data-word-revision]'))return;const s=services.find(s=>a.getAttribute('href')==='/service/'+s[0]);if(!s)return;const image=a.querySelector('img');if(image)image.src=base+s[3];const h=a.querySelector('h3'),p=a.querySelector('p');if(h)h.textContent=s[1];if(p)p.textContent=s[2]});
 if(path==='/service/ads')document.querySelectorAll('.svcdetailhero_button-navy .svcdetailhero_button-label').forEach(e=>{if(!e.closest('[data-word-revision]'))e.textContent='マーケティング支援を見る'});
